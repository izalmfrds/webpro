...
$kategori_list = 'SELECT
					...
					kategori.kategori,
					kategori.id_kategori';
$list_kategori = $con->query($kategori_list) or die($con->error);
...
$list_terkini = $con->query($terkini) or die ($con->error);

$list_populer = $con->query($populer) or die ($con->error);
?>
					...
					<div class="col-md-4 sidebar">
						<!-- <div class="sidebar-item kategori">
							...
							<?php if (isset($_GET['kat']) && $data_kat['id_kategori'] == $_GET['id_kategori'] ) { ?>
								<li class="active">
									<a href="<?php echo $base_url."kategori.php?id_kategori=".$data_kat['id_kategori']."&amp;kat=".strtolower($data_kat['kategori']); ?>">
						...
						<div class="sidebar-item">
							...
							<?php while ($populer_list = $list_populer->fetch_array()) { ?>
								<?php if($populer_list['status'] == 'ya') { ?>
								<li>
									<a href="<?php echo $base_url."Berita/berita.php?id_berita=".$populer_list['id_berita']."&amp;judul=".strtolower(str_replace(" ", "-", $populer_list['judul'])); ?>">
						...
						<div class="sidebar-item">
							...
							<?php while ($terkini_list = $list_terkini->fetch_array()) { ?>
								<?php if($terkini_list['status'] == 'ya') { ?>
								<li>
									<a href="<?php echo $base_url."Berita/berita.php?id_berita=".$terkini_list['id_berita']."&amp;judul=".strtolower(str_replace(" ", "-", $terkini_list['judul'])); ?>">
