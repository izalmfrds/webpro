...
if (isset($_POST['btn_simpan'])) {
    ...
    $txt_username = $con->real_escape_string($_POST['txt_username']);
    $txt_nmlengkap = $con->real_escape_string($_POST['txt_nmlengkap']);
    $txt_deskripsi = $con->real_escape_string($_POST['txt_deskripsi']);
    ...
    if (count($message)==0) {
    	...
    	$qry_update = $con->query($sql_update);
    	if (!$qry_update) {
    		die ($con->error);
    	} else { ?>
